...
constraint workflow_main_fk3 foreign key (sign_num) REFERENCES workflow_form (sign_num)
);
...
mem_name varchar2(300) not null,
constraint workflow_sign_pk primary key (sign_num),
constraint worklfow_sign_fk1_ foreign key (flow_num) REFERENCES workflow_flow (flow_num),
...
mem_num number not null,
sign_no  varchar2(30) not null,
sign_name varchar2(300) not null,
constraint workflow_test_pk primary key (flow_num),
constraint worklfow_test_fk1_ foreign key (mem_num) REFERENCES member (mem_num)
);
