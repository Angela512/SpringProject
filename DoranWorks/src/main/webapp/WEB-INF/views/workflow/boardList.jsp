...
<div class="page-main">
	<h2 style="padding:0 0 0 10px; margin:0 0 30px 0;">전자결재</h2>
...
<!-- 	<input type="button" value="목록" onclick="location.href='sign.do'"> -->
	<form action="list.do" id="search_form" 
		...
		<ul class="search">
			...
			<li>
				<select name="keyfield" id="keyfield" style="height:32px">
			...
			<li>
				<input type="search" name="keyword" id="keyword"
				               value="${param.keyword}" style="height:32px">
			</li>
			<li>
...
<input type="button" value="+ 새 결재 진행 " class="btn btn-outline-primary"
		          onclick="location.href='write.do'" style="height:70px; transform:translateY(-20px)">
...
<div style="width:200px; height:70px;  display:inline-block; border:4px solid #9370DB; border-radius:0.25rem;">
	...
	<p style="text-align:center;  color:#9370DB; font-size:20px;"><b>${cnt1}</b></p>
</div>
<div style="width:200px; height:70px;  display:inline-block; border:4px solid #09E81F; border-radius:0.25rem;">
	...
	<p style="text-align:center;  color:#09E81F; font-size:20px;"><b>${cnt2}</b></p>
</div>
...
<div style="width:700px; height:80px; display:inline-block; border:6px solid gray; border-radius: 50px;">
	...
	<a href="detail.do?flow_num=${flow.flow_num}" style="display:inline-block;  transform:translateY(-30px); width:350px"><fmt:formatDate value="${flow.flow_date}" pattern="yyyy/MM/dd"/> ${flow.flow_title}</a>
